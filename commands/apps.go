...
Only basic information is included with the text output format. For complete app details including the app specs, use the JSON format.`,
	...
	AddStringSliceFlag(listJobInvocations, doctl.ArgAppJobName, "", []string{}, "The job component names to filter job invocations for. If not provided, all job invocations for given app are returned.")
...
func RunAppsListJobInvocations(c *CmdConfig) error {
	...
	jobNames, err := c.Doit.GetStringSlice(c.NS, doctl.ArgAppJobName)
	if err != nil {
		return err
	}
